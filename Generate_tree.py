...
parser.add_argument("--inletknown")
parser.add_argument("--inlet1_x")
...
n_seed = 32000
weight = float(args.weight) #g but is a proxy for cm3 since density of water is 1 g/cm3
...
print(f"Area in mm2: {placenta_area:.2f}")
#######################################################################
...
inletknown = args.inletknown
inlets = np.array([[float(args.inlet1_x), float(args.inlet1_y)], [float(args.inlet2_x), float(args.inlet2_y)]])
px_g, coord, art_nodes, art_elems,inlets = skel2graph(skel_art, outputfilename, debug_export_all,inlet_type,arteries, inletknown,inlets)
