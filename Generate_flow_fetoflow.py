...
outlet_pressure, inlet_flow, inlet_pressure = 2660, 4166.7, 6650
...
if adjusted_radi:
    G = create_geometry(nodes, elems, umbilical_artery_radius, decay_factor, umbilical_vein_radius, decay_factor_vein,arteries_only=arteries_only, fields=radii, single_umbilical_vein=False)
else:
...
export_region_as_csv(G, 'order',output_flow_dir+sample_number+'_order.csv', chorion_elems = chorion_elems[:,0], order_interest= [9,8])
export_region_as_csv(G, 'chorion',output_flow_dir+sample_number+'_chorion.csv', chorion_elems = chorion_elems[:,0], order_interest= [9,8])
